...
public class CLUI {
    ...
    public CLUI(ServiceFactory store, int secret) {
        this.scanner = new Scanner(System.in);
        serviceFactory = store;
        gson = new Gson();

        // System.out.println("1/2 shortage/periodic");
        ...
        // -------------------------------------------------------------------------------------------------------/
            ...
            try {
                ...
                productscontroller.getInstance().buildProduct(1, "milk", -1, 5, 4, 1, "milk",
                        Categorycontroller.getInstance().getCategories().get("milk"));
                productscontroller.getInstance().buildProduct(2, "Bamba", -1, 5, 4, 1, "milk",
                        Categorycontroller.getInstance().getCategories().get("milk"));
                productscontroller.getInstance().buildProduct(3, "Fanta", -1, 5, 4, 1, "milk",
                        Categorycontroller.getInstance().getCategories().get("milk"));
                productscontroller.getInstance().buildProduct(4, "Maltesers", -1, 5, 4, 1, "milk",
                        Categorycontroller.getInstance().getCategories().get("milk"));
                productscontroller.getInstance().buildProduct(5, "Tuna", -1, 5, 4, 1, "milk",
                        Categorycontroller.getInstance().getCategories().get("milk"));
    ...
    public void addSupplier() {
        ...
        handle_response(serviceFactory.addSupplier(company_name, phone_number, company_id, bank_account, contactList),
                true);
    ...
    private void addContract() {
        ...
        System.out.println(
                "Enter whether the supplier of this contract is responsible of delivering the orders or now: ");
        System.out.println("1. true");
        System.out.println("2. false");
        ...
        System.out.println(
                "Enter the days it takes the supplier of this contract to prepare an order and order it  once receiving one");
    ...
    public void removeContract() {
        while (true) {
            ...
            int id = scanner.nextInt();
            if (id == -1 || !handle_response(serviceFactory.removeContract(id), true))
                break;
            System.out.println("\n" + "Enter a valid id..");
        }
    }
    ...
    public void editContract() {
        ...
        switch (choice) {
            ...
            case 2:
                ...
                int choice_quant = scanner.nextInt();
                scanner.nextLine();
                System.out.println("Enter the value of the discount");
                int dis = scanner.nextInt();
                scanner.nextLine();
                Character c = (choice3 == 1) ? 'p' : 'v';
                handle_response(serviceFactory.add_discount(suppid, items_map.get(itemChoice).getId(), choice_quant,
                        dis, false, c), true);
                break;
            case 3:
                System.out.println("Enter quantity to add discount to it");
    ...
    public void Prepare_Order() {
        ...
        handle_response(serviceFactory.add_PeriodicOrder(supp_name, supplier_id, address, contact_number, items, days),
                true);
    }
